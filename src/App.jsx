      ...
      */
      ...
      if (list.length) {
        ...
        if (normalisedQueue.length) {
          // If the clicked item is not present in the API list,
          // keep it at the front so Next/Previous can still work.
          const hasCurrent = normalisedQueue.some(
            (item) => String(item.id) === String(normalised.id)
          );

          const finalQueue = hasCurrent
            ? normalisedQueue
        ...
        if (!exists) {
          const nextQueue = [...queueRef.current, normalised];
